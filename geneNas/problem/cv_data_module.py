...
class CV_DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if not self.cache_dataset:
            print('not cache')
            self.dataset = {}
            self.dataset['train'] = split_stratify(
                tensor_dataset(getattr(torchvision.datasets, self.dataset_names[self.task_name])(root='./cifar10_data',
                                                                                                 train=True,
                                                                                                 download=True,
            ...
            self.dataset = {}
            self.dataset['train'] = split_stratify(tensor_dataset(
                getattr(torchvision.datasets, self.dataset_names[self.task_name])(root=self.cached_dataset_filepath,
                                                                                  transform=self.convert_img)),
                                                   self.train_percentage)
...
class CV_DataModule_RWE(CV_DataModule):
    ...
    def setup(self, stage):
        if not self.cache_dataset:
            ...
            self.dataset['test'] = getattr(torchvision.datasets, self.task_name.upper())(
                root=self.cached_dataset_filepath,
                train=False,
                transform=self.convert_img,
                target_transform=self.onehot
                )
